...
func saveSourceFiles(images []*pb.DownloadImagesRequest) []OriginalImage {
	...
	for i := range images {
		...
		go func(i int) {
			...
			newFilePath := "./files/" + y + "/" + m + "/" + d + "/" + uuid + "/" + "img/"
			...
			//создаем файл для последующего сохранения туда картинки
			...
			// берем мьютекс для корректного добавления в слайс нашего изображения
			mu.Lock()
