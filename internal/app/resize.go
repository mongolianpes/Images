...
func resizeAndSave(paths []OriginalImage) []string {
	...
	var mu sync.Mutex
	//создаем результирующий слайс путей наших файлов чтобы потом отдать клиенту
...
func resizeImage(path OriginalImage) []string {
	...
	case "png":
		for i := range path.Lenght {
			...
			//если картинка станет больше по пиксели в результате изменения размера, то не изменяем размер
			bounds := imgPng.Bounds()
			if path.Lenght[i] >= int32(bounds.Dx()) && path.Width[i] >= int32(bounds.Dy()) {
				return []string{}
			}
